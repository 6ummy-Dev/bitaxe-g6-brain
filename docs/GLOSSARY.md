...
**last_efficiency** The most recent `power_w / hr_ths` ratio (W/TH). Only updated when `power_w` is within sanity bounds **and** the sample was not rejected by the 3-sigma outlier gates — on fail-closed and outlier-rejected paths the field retains its last known-good value rather than reporting a garbage ratio.
...
**Under-excitation / Identifiability** The six-coefficient quadratic surface can only be identified if the operating point varies; at a fixed `(f, v)` only one parameter direction is excited, five stay undetermined, and the covariance ill-conditions. `G6BrainTelemetry.model_under_excited` (true once past cold start and `cov_condition > G6_EXCITATION_COND_WARN`) flags when this makes the optimizer's recommendations untrustworthy despite a high `model_quality`. `cov_condition` is the underlying certified condition-number bound (Gershgorin upper bound while the covariance is near-diagonal; Cholesky definiteness certificate plus diagonal-ratio lower bound once updates have introduced off-diagonal mass; the ~1e12 sentinel is reserved for certified loss of positive-definiteness). Both are observability-only (no control effect). The on-device exploration that supplies the needed variation is tracked in `docs/ROADMAP.md`.
