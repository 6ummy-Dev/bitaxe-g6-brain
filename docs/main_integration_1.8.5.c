// Fixed brain_task integration for ESP-Miner 1.8.5 revised
...
static void brain_task(void *pvParameters)
{
    ...
    while (1) {
        // Fixed telemetry extraction for 1.8.5 revised:
        ...
        // In full 1.8.5 merge, extend SYSTEM_MODULE or use asic_get_frequency() / voltage from power task.
        float f = 780.0f;
        float v = 1220.0f;
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Welcome to G6 AxeOS 1.8.5 revised - FOSS || GTFO! + Brain v%s", G6_BRAIN_VERSION);
    ...
    if (system_init_ret == ESP_OK) {
        ...
        g6_brain_init(&g6_brain); // Initialize G6 Brain (1.8.5 revised - now with real telemetry hook)
