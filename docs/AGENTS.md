...
3. **Thermal Protection**: Hard `G6_TEMP_CEILING` with proactive scaling. VR has its own independent ceiling (`G6_VR_TEMP_CEILING`) and proactive margin.
   ...
   - **Tiered response structure** — VR's helper handles both the hard-ceiling tier (≥ ceiling: aggressive frequency + voltage derate) and the proactive tier (> ceiling − margin: voltage-only derate) in a single function, because VR temperature only ever affects setpoints. ASIC's hard-ceiling logic lives in `is_thermal_safe()` and gates RLS *learning* on a separate path (item 2 above); the ASIC proactive helper only handles the proactive tier on the setpoint path. Both helpers guard `brain != NULL`, finiteness of the input temperature, and finiteness/positivity of the ceiling at entry. The margin guard differs by necessity: the ASIC helper owns only the proactive tier, so a non-finite margin may safely no-op the whole function at entry, while the VR helper owns both tiers and therefore guards its margin on the proactive branch only — an entry return there would wrongly disable the margin-independent hard-ceiling tier.
