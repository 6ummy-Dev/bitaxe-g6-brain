...
- **Returns:** `ESP_OK` on successful execution, or `ESP_ERR_NVS_NOT_INITIALIZED` if the NVS subsystem has not been initialized by the host application.
...
* **Returns:** - `ESP_OK` on successful execution, **including** when out-of-range sensor values (e.g., frequency bounds, voltage limits, or power sanity) are caught and handled by the fail-closed safety layer.
* `ESP_ERR_INVALID_ARG` strictly for fundamentally invalid inputs that would crash mathematical models (non-finite values, NaN, or negative hashrate).
...
* `safety_status` — `G6SafetyStatus` enum indicating the current operational state (or most recently triggered safety intervention)
