...
- **Purpose:** Returns a *certified one-sided bound* on the tracking covariance matrix's 2-norm condition number. Two regimes: while the covariance is near-diagonal (fresh model), Gershgorin disc bounds give a true **upper** bound (`max_i(P_ii + Σ_{j≠i}|P_ij|) / min_i(P_ii − Σ_{j≠i}|P_ij|)`), which reads ≈1 at the `1e5·I` cold start. Once updates have introduced off-diagonal mass — which the correlated quadratic basis does immediately — the Gershgorin lower bound is uninformative, so definiteness is certified directly by a Cholesky factorization attempt and, when positive-definite, the diagonal ratio `max_i P_ii / min_i P_ii` is returned as a true **lower** bound (for symmetric `P`, `λ_min ≤ P_ii ≤ λ_max`). Exceeding a threshold on a lower bound is therefore positive evidence of ill-conditioning, never a false alarm from a loose bound. The `RLS_P_CLAMP_MAX/RLS_P_CLAMP_MIN` sentinel (~1e12) is returned **only** for a certified non-positive-definite covariance — the state a strictly fixed operating point drives the estimator into shortly before its automatic recovery. Healthy reference values: ≈1 fresh; low single digits on a converged, well-excited model.

### `esp_err_t g6_brain_self_test(const G6BrainState *brain)`

- **Purpose:** Validates matrix symmetry, diagonal range, and covariance conditioning — including a Cholesky positive-definiteness certificate via the condition estimate (see `g6_brain_get_cov_condition` above) — to determine if estimators are running normally or in a degraded state. Returns `ESP_OK` when healthy, `ESP_FAIL` when any check fails, or `ESP_ERR_INVALID_ARG` if `brain` is `NULL`. A healthy brain passes at every reachable operating state, fresh or converged; `ESP_FAIL` means positive evidence of degradation (out-of-range diagonal, asymmetry, certified ill-conditioning, or certified indefiniteness).
...
| `cov_condition` | `float` | Certified bound on the hashrate covariance condition number — identical to `g6_brain_get_cov_condition()`, surfaced in the snapshot so health can be read in one call. ≈1 for a fresh model; low single digits on a converged, well-excited one; the ~1e12 sentinel appears only when the covariance is certifiably non-positive-definite, which a strictly fixed operating point produces shortly before the automatic `G6_SAFETY_P_MATRIX_SINGULAR` recovery |
| `model_under_excited` | `bool` | Observability flag: `true` once past cold start **and** `cov_condition > G6_EXCITATION_COND_WARN`. Because the estimate is a certified bound, the flag now fires only on positive evidence: either the diagonal-ratio lower bound exceeds the threshold (partial excitation — variance collapsed along visited directions while unvisited ones retain it) or the covariance is certifiably degenerate (strictly fixed point). It signals that the optimizer's recommendations are not identifiability-backed — even when `model_quality` reads high (quality measures fit at the visited point, not identifiability). Telemetry only; changes no control behavior. See [Model identifiability](#model-identifiability--under-excitation) |
...
`model_under_excited` fills that gap for the partial-excitation band: variance collapses along directions the operating point has visited while unvisited directions retain their large prior, and the certified diagonal-ratio lower bound crosses the threshold on that contrast. One honest limitation at the *strict* fixed-point extreme: there, the covariance degenerates all the way to certified indefiniteness and the brain self-heals by cold-start recovery — on the host reference model roughly every ~37 accepted samples — which resets `update_count` and re-enters cold start, so the flag is only visible in the short window between certified degeneracy and the recovery. For detecting strictly fixed-point operation, the periodic `G6_SAFETY_P_MATRIX_SINGULAR` events (and their WARN log line) are the reliable signature; the flag's discriminating value is the in-between regimes. When the flag is `true`, do not act on `best_f`/`best_v` as if they were optimized — the operating point needs to vary so the surface can be identified. The flag and `cov_condition` are observability only and change no control behavior; the threshold is a conservative starting value pending closed-loop field calibration. The full account, and the planned on-device exploration that supplies the needed variation, are in `docs/ROADMAP.md`.
