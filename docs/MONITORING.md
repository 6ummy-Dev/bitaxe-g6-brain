# MONITORING.md — Real-Time G6 Brain Observability (v1.0.0-beta3)
...
```bash
grep -E "G6_BRAIN|PROACTIVE THERMAL|HIGH ERROR RATE|Outlier Rejected|NVS fingerprint|Quality=|Mode=|G6_TELEMETRY" /dev/ttyUSB0
...
*(Note: In efficiency mode, the same thresholds apply to `power_model_quality`)*
