...
## [1.0.0-beta7.5] — 2026-07-02

Intensive-QA point release on top of beta7.1, assembled across two pre-tag QA rounds — 2026-06-11 (suite-runtime integrity, NVS-load hardening, doc truth) and 2026-07-02 (execution-QA of the shipping estimator on a host harness with behavioral probes: conditioning-diagnostics rewrite, telemetry integrity, NVS frame verification, dependency hygiene). No control behavior changes under supported configurations and no public API signature changes (one additive named constant, `G6_SLEW_STEP_MV_MAX`); the *semantics* of the observability diagnostics (`g6_brain_get_cov_condition`, `self_test`, `model_under_excited`) are corrected — they now report certified evidence instead of a saturated sentinel. The first round's three substantive classes: test-suite **runtime** integrity (the suite as shipped fails two cases at runtime — one from NVS cross-test contamination, one from a surviving integer-macro float assert — and any partial/selective run leaves contaminating state in flash for the next session; all invisible under compile-only CI), NVS-load hardening that makes two long-documented contracts actually true (the `NVS erase failed` ERROR line and bad-blob auto-erase for oversized blobs), and a doc-truth sweep. Suite grows 41 → 47 cases across the two rounds. Version stamps bumped to `v1.0.0-beta7.5` across code and docs.

### Fixed
- **Covariance conditioning estimate false-alarmed on every reachable post-update state — `self_test` permanently `ESP_FAIL`, `model_under_excited` inverted (execution-QA finding FQA-3).** The minimum Gershgorin disc `min_i(P_ii − Σ|P_ij|)` goes non-positive on the very first accepted update — the correlated quadratic basis puts off-diagonal mass into `P` immediately — and the estimate conflated "lower bound uninformative" with "matrix ill-conditioned," returning the ~1e12 sentinel forever after in **every** regime. Measured on the shipping code: a fully converged, richly excited model (840 accepted updates, `model_quality` 1.0000, true eigen condition number **7.3**) read `cov_condition = 1e12`, failed `self_test` against its 5e5 ceiling, and reported `model_under_excited = true` — the exact inversion of the beta7 feature's intent — while the strictly-fixed-point regime the feature was meant to expose read the identical sentinel. In practice the metric was binary (≈1 before the first update, 1e12 after), so the beta7 plan to "calibrate the warn threshold from field data" had nothing to calibrate. Replaced with a **certified two-path estimate**: Gershgorin `hi/lo` as a true upper bound while the covariance is near-diagonal (fresh model reads ≈1); otherwise a Cholesky factorization attempt certifies definiteness directly and, when positive-definite, the diagonal ratio `max P_ii / min P_ii` is returned as a true lower bound (`λ_min ≤ P_ii ≤ λ_max` for symmetric `P`) — threshold crossings are now positive evidence, and the sentinel is reserved for certified indefiniteness, the state a strictly fixed operating point drives `P` into a few samples before the `xPx < 0` recovery. Post-fix measurements: the converged healthy model reads 3.25 with `self_test` `ESP_OK` and the flag clear; a 5040-update excited soak settles at ≈2.2 with zero false flags; at a strict fixed point the flag and `self_test` report certified degeneracy ~5 samples before the automatic recovery. Consumers and thresholds (`G6_EXCITATION_COND_WARN` 1e5, `self_test` ceiling 5e5) unchanged. *(Correction to the [1.0.0-beta7] entry per the changelog-as-net-truth rule, dated 2026-07-02: that entry claimed the new fields "surface the wide intermediate band where recommendations are not yet trustworthy" — execution QA showed the metric as shipped could not, since it saturated at the sentinel from the first update; the intermediate band only became observable with this release's certified estimate.)* Root of the coverage hole: no test ever queried `cov_condition`/`self_test` on a covariance that had absorbed a real update — the healthy fixtures were fresh diagonals, the degraded ones hand-blown diagonals, neither a reachable operating state. Closed by three of this round's new tests.
- **`last_efficiency` accepted 3-sigma-rejected hashrate samples (FQA-5).** The tail telemetry gate checked only `hr_ths > 0` and power sanity, not the sample's own outlier verdict: a finite, in-bounds 500 TH/s spike against a ~1.2 TH/s model was rejected by the learning path yet still wrote `power/hr = 0.03 W/TH` into `last_efficiency` — a garbage ratio from a sample the brain itself refused to learn from, violating the hr-side of `GLOSSARY.md`'s retains-last-known-good contract (which held power-side only). The gate now also requires the tick's hashrate/power outlier verdicts to be clean; the verdict flags are declared before any fail-closed `goto` so every path reaching the safety layer sees initialized values, and paths that fail closed before the outlier test keep their previous telemetry behavior.
- **VR proactive margin: explicit finiteness guard, and the docs' guard-symmetry claim made true (FQA-2).** `g6_safety_proactive_vr_thermal_scale()` never guarded `vr_temp_proactive_margin`, while `docs/AGENTS.md` item 3 and the ASIC helper's comment both claimed an identical margin entry-guard in the two helpers. A NaN margin already no-opped the proactive tier via NaN-comparison semantics, but an Inf margin drove the proactive threshold to −∞ and derated voltage every tick. The guard now sits on the **proactive branch** — an entry return would wrongly disable the margin-independent hard-ceiling tier, which the VR helper owns but the ASIC helper does not — and `AGENTS.md` plus the ASIC comment now state the true, deliberately asymmetric pattern. Unreachable via supported configuration (the margin has no setter and comes from an integer Kconfig default): defense-in-depth plus doc truth.
- **NVS fingerprint payload-size field is now verified on load.** Written since the schema's introduction, never read: a frame with the right version and right total blob length but a corrupted interior header loaded silently. The loader now checks it and routes mismatches through the same WARN/erase/cold-start path as every other bad-blob case, with a new documented log line (`NVS payload size field mismatch (got X, expected Y) — erasing`) added to `MONITORING.md`'s verbatim table.
- **Test suite: unused-variable warning removed (`vn` in the Dinkelbach J/TH test).** `v` is pinned at `BM1370_V_CENTER` in that fixture, so `vn = 0` and every `vn` term vanishes; the manual J/TH expressions never referenced the variable, and the dead declaration warned under `-Wall` — i.e., in ESP-IDF CI on every build. Replaced with a comment stating why the `vn` terms are omitted.
- **Test suite: NVS cross-test contamination — one deterministic failure per full run, and a warm-contaminated session start after any partial run.** The save/load round-trip test persists a fingerprint (`theta[0]=42`, `P[0][0]=12345`, …), `tearDown` never erased it, and `setUp → g6_brain_init()` warm-starts from NVS — so every test between the round-trip and the bad-size-blob test inherited that blob. "Cold start flag clears after sufficient updates" fails deterministically on its first assert (`cold_start` arrives `false` from warm-start); the other tests in the contaminated window survive only by fixture accident. A full pass is *accidentally* self-cleaning — the bad-size-blob test downstream overwrites the same key and its load-path erase removes it — so consecutive full runs repeat the same single failure. But any **partial or selective run** that executes the round-trip without reaching the bad-size test (the ESP-IDF Unity runner's interactive menu makes selective runs routine) leaves the blob in flash, and the next session starts warm: "g6_brain_init initializes correctly" and "Cold start flag clears" both fail at session start. `setUp` now erases the fingerprint key before `g6_brain_init()`, so every case cold-starts clean-room regardless of order, selection, or what a previous session left behind; the round-trip and blob tests are unaffected (they write and read within a single test body). Never observed because CI compiles but does not execute — the latest in the line of latent runtime failures (beta6.5's covariance pair, beta7.1's slew no-op and integer assert) that the manifesto §3 execution requirement exists to catch. *(Correction, 2026-06-12, pre-tag: the original wording of this entry overclaimed the blast radius — it listed the beta7 observability test as a second in-order failure and predicted first-test failures on every re-run, missing the bad-size test's incidental mid-suite cleanup. Host execution of the pre-fix suite settled it: one contamination failure per full run, stable re-runs, with the partial-run hazard above as the real persistent-state risk. Recorded per the changelog-as-net-truth rule.)*
...
### Changed
- **NVS save interval is now tick-rate-independent: `pdMS_TO_TICKS(50 min)`.** The raw constant `300000UL` equaled 50 minutes only at the ESP-IDF default 100 Hz tick and silently became 5 minutes at `configTICK_RATE_HZ = 1000`. Fifty minutes is the deliberate value: gentle on NVS flash wear, and — since it exceeds the strict-fixed-point recovery cycle (host reference ~37 accepted samples ≈ 25 min at a 40 s telemetry cadence) — a freshly re-cold-started model is never persisted over a previously learned fingerprint at typical cadences. Under the old constant at 1000 Hz, a learned fingerprint could be overwritten by an amnesic one within minutes of settling at a fixed operating point.
- **Dead includes and a phantom dependency removed.** `esp_timer.h` from `g6_brain.c` and `esp_timer` from `CMakeLists.txt` `PRIV_REQUIRES` (no `esp_timer` usage anywhere in the component), plus the redundant `freertos/portmacro.h` include (already provided via `FreeRTOS.h`). No behavior change.
- **Identifiability and Phase-1 documentation corrected to measured behavior (FQA-4/FQA-6 doc set).** `docs/INSTALL.md` Phase 1 no longer claims RECOMMEND "never mutates `best_f`/`best_v`" — safety derates adjust setpoints in every mode (the same falsehood class the beta7.1 sweep fixed in `SAFETY.md`/`README.md` but missed here) — and no longer frames Phase 1 as model-building: at a fixed operating point the six-term model cannot become identified; the covariance periodically degenerates and the brain self-heals by design, so the recovery WARN line and the `update_count` sawtooth are now documented as *expected* fixed-point behavior rather than a fault, with the measured host cadence (~37 accepted samples) recorded in `MONITORING.md`. `docs/API.md`, `docs/MONITORING.md`, `docs/GLOSSARY.md`, and `docs/SAFETY.md` updated to the certified-estimate semantics above, including the honest limitation that at the strict fixed point the under-excitation flag is visible only in the short pre-recovery window — the periodic `G6_SAFETY_P_MATRIX_SINGULAR` events are the reliable signature there; the flag's discriminating value is the partial-excitation band.
...
### Added
- **Five tests (suite: 42 → 47), three of which fail against the pre-fix code as regression proofs.** "self_test and cov_condition stay healthy on a converged, well-excited model" (pre-fix: sentinel / `ESP_FAIL` / flag-true on the healthiest reachable state); "last_efficiency retains last known-good value when the sample is a rejected outlier" (pre-fix: 0.03 W/TH garbage ratio written); "NVS fingerprint with corrupt in-frame size field is rejected and erased" (pre-fix: corrupt frame accepted silently). Plus two contract tests: "Under-excitation flag fires on certified degeneracy before fixed-point recovery" (flag/`self_test`/recovery ordering at a strict fixed point, deliberately tick-index-agnostic for FPU portability) and "self_test detects an indefinite covariance that passes diagonal and symmetry checks" (the definiteness certificate's reason to exist — a symmetric, in-range, indefinite matrix the older checks cannot catch).
...
> **Note:** A first for this project: the suite **was executed before tagging** — on a host harness (x86-64, GCC 13; the shipping `g6_brain.c` and `test_g6_brain.c` compiled unmodified against mock NVS/FreeRTOS/Unity layers that preserve Unity's integer-macro semantics and real flash persistence). Results: **42/42 green, twice consecutively on un-erased mock flash**, and 42/42 again when the flash is pre-seeded with a stale contaminating fingerprint (the partial-run scenario). Reverting the two test fixes in the same harness reproduces the pre-fix behavior deterministically — "Cold start flag clears" fails from contamination and the noiseless-convergence test fails on the integer-macro assert, every run; with pre-seeded flash the init test fails too — empirically confirming both bug mechanisms (and correcting this entry's original failure inventory, see above). Host execution supplements but does not satisfy the standing gate: the on-target run (`idf.py -T g6_brain build flash monitor`, QEMU or Gamma) remains the environment of record, and the closed-loop AUTO soak is still required before any PROD tag.

> **Note (2026-07-02 fix round):** The full battery was re-executed on the host harness after this round's fixes: **47/47 green** on fresh flash, on un-erased re-run, with pre-seeded stale-fingerprint flash, and in the partial-run-then-fresh-session scenario; ASan+UBSan clean twice consecutively; Valgrind memcheck zero errors; `-Wconversion -Wdouble-promotion -Wshadow -Wfloat-equal` at `-O2` produce zero warnings on `g6_brain.c`; `gcc -fanalyzer` reports only the previously-confirmed false positive. The three regression tests were additionally run against the *pre-fix* sources and fail exactly as documented. Behavioral probes against the fixed tree: the converged 840-update model reads `cov_condition` 3.25 (true eigen condition 7.3) with `self_test` `ESP_OK`; a 5040-update excited soak settles at ≈2.2 with zero false flags and zero recoveries; the strict-fixed-point amnesia cycle is unchanged at ~37 samples — by design, since the exploration cycle that dissolves it is the next release's mandate. The on-target run and closed-loop AUTO soak remain the standing pre-tag gate.
