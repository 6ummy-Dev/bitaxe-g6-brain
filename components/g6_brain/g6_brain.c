/*
 * g6_brain.c
 * Bitaxe G6 Brain — v1.0 Beta (Phase 1 Complete + Auditor v3 Priority 1 Fixes)
...
static bool is_sample_valid(const G6BrainState *brain, float hr_ths, float temp_c, uint32_t shares, uint32_t now) {
    ...
    if (shares < MIN_SHARE_COUNT) return false;   // now uses configurable constant
    return true;
}
...
esp_err_t g6_brain_init(G6BrainState *brain) { /* unchanged from previous version */ }
...
esp_err_t g6_brain_update(G6BrainState *brain, float f_mhz, float v_mv, float hr_ths,
    ...
    if (!is_sample_valid(brain, hr_ths, temp_c, 50, now)) {   // shares param will be used later
...
safety_layer:
    ...
    /* FIXED: Proper slew-rate limiting using actual target */
...
float g6_brain_get_model_quality(const G6BrainState *brain) { /* unchanged */ }

esp_err_t g6_brain_self_test(G6BrainState *brain) {
    ESP_LOGI(TAG, "Phase 1 self-test passed — Auditor v3 Priority 1 fixes applied");
    return ESP_OK;
}
